...
for kat in range(len(Folders)):
    ...
    os.mkdir(FolderWholeCell)
    FileNames=gb.glob('Filament*.csv')
    for katN in FileNames:
        DataRoot=pd.DataFrame.from_csv(katN)
        CellFilIds=rnd.sample(FileNames,CellFilNum-1)
        
        #print(katN)
        Count=0
        for katNbd in CellFilIds: 
        ...
        DataRoot.to_csv(FolderWholeCell+katN[0:len(katN)-4]+'_Bundle.csv')
    os.chdir('../')
